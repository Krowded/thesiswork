...
function [vertices, extraVertices] = curveVerticesCombined(vertices, curves, extraVertices)
    if nargin < 3
        extraVertices = [];
    end
    ...
    totalChange = zeros(size(vertices));
    ...
    for i = 1:length(curves)
        ...
        tempVert = vertices + y.*curvingDirection;
...
%             extraVertices = extraVertices + distanceRatio.*y.*curvingDirection - distanceRatio.*baseY*curvingDirection;
    ...
    %Apply everything
    vertices = vertices + totalChange;
    if ~isempty(extraVertices)
        extraVertices = extraVertices + totalChangeExtra;
    end
    
    if any(any(isnan(vertices))) || any(any(isnan(extraVertices)))
